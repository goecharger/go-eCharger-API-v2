...
## Über die App die OCPP Verbindung aktivieren und prüfen
...
| ocppti   | OCPP Transaktion ID             | null or int     | R/W  | Status    |
...
| ocppd    | OCPP Testkarte ID (wird benutzt wenn keine Karte hingehalten wird und es schon erlaubt ist. | string | R/W | Config |
| ocppr    | OCPP rotiere Phasen am Charger  | bool            | R/W  | Config    |
...
| ocpprl   | OCPP remote logging (normalerweise durch go-e Support aktiviert um Debugging zu ermöglichen) | bool | R/W | Config |
| ocppcs   | OCPP connector status (0=Available, 1=Preparing, 2=Charging, 3=SuspendedEVSE, 4=SuspendedEV, 5=Finishing, 6=Reserved, 7=Unavailable, 8=Faulted) | int | R | Config |
| ocppt    | OCPP accept time wie vom Backend in BootNotification-bestätigug| bool | R/W | Config |
| ocppio   | OCPP als "inoperable" gesetzt (kann vom Backend mittels ChangeAvailability Nachricht verändert werden) | bool | R/W | Config |

## Wie meldet sich der Charger am Backend?
...
| DataTransfer                  | Core                       | Implementiert (PRO CABLE ME) |
...
Die OCPP Message `GetConfiguration` wird verwendet, um einzelne oder gleich mehrere Konigurationswerte vom Charger zu holen. Die OCPP Message `ChangeConfiguration` kann einzelne oder auch mehrere Konfigurationswerte am Charger verändern.
